...
@router.get("/benhnan")
def get_benh_nhan(benh_nhan_id: int, db: Session = Depends(get_db)):
    benh_nhan = db.query(BenhNhan).filter(BenhNhan.id_benh_nhan == benh_nhan_id).first()
    if not benh_nhan:
        raise HTTPException(status_code=404, detail="Benh nhan not found")
    return benh_nhan
    db.commit()
    return {"message": "Da lay benh nhan thanh cong"}
